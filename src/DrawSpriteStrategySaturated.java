...
public class DrawSpriteStrategySaturated extends DrawSpriteStrategyBase
{
    ...
    public void drawImage(java.awt.Image img, int x, int y, int width, int height)
    {
        ...
        for (int i = 0; i < buf.getWidth(); i++)
        {
            for (int j = 0; j < buf.getHeight(); j++)
            {
                ...
                Color sat = saturateColor(c, 1.5f);
    ...
    {
        Color sat = saturateColor(color, 1.5f); // 1.5x saturation
